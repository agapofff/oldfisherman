...
?>
        ...
        <?php 
            if ($count > 0) { 
        ?>
                <span class="cart-content-count position-absolute top-0 start-100 translate-middle badge rounded-circle bg-primary border border-light fw-light d-flex justify-content-center align-items-center p-2 mt-2 ms-n1">
...
if (!function_exists('bs_woocommerce_breadcrumbs')) :
  ...
  function bs_woocommerce_breadcrumbs() {
    return array(
      'delimiter'   => '',
      'wrap_before' => "<nav aria-label='breadcrumb' class='wc-breadcrumb breadcrumb-scroller mb-4 px-3 bg-light rounded'>
...
function my_woocommerce_widget_shopping_cart_button_view_cart() {
  echo '<a href="' . esc_url(wc_get_cart_url()) . '" class="btn btn-outline-primary d-block mb-2" data-pjax="0">' . esc_html__('View cart', 'woocommerce') . '</a>';
}
function my_woocommerce_widget_shopping_cart_proceed_to_checkout() {
  echo '<a href="' . esc_url(wc_get_checkout_url()) . '" class="btn btn-primary d-block" data-pjax="0">' . esc_html__('Checkout', 'woocommerce') . '</a>';
}
